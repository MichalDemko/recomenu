...
def home():
    ...
    if request.method == 'POST':
        ...
        if len(food) < 1:
            ...
            categoriesId = (
                int(x.split('_')[-1]) for x in request.form.keys() if x.startswith('category_'))
            categories = [Category.query.get(x) for x in categoriesId]
            if not categories:
                flash('No categories selected', category='error')
                return render_template("home.html", user=current_user)
            new_food = Food(name=food, user_id=current_user.id,
...
def menu():
    ...
    if mydate:
        ...
        menu = get_menu(current_user, mydate.isocalendar().year,
                        mydate.isocalendar().week)
    else:
        ...
        if 'categoryform' in request.form:
            ...
            if len(category) < 1:
                ...
                new_category = Category(
                    name=category, value=value, immediateValue=10, user_id=current_user.id)
